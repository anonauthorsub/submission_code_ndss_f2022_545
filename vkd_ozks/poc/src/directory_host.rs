...
use vkd::errors::VkdError;
use vkd::storage::types::*;
use vkd::storage::Storage;
use vkd::{Directory, EpochHash};
...
async fn get_root_hash<S, H, V>(
    ...
    o_epoch: Option<u64>,
) -> Option<Result<H::Digest, VkdError>>
where
    S: Storage + Sync + Send,
    H: Hasher,
    V: VRFKeyStorage,
{
    if let Ok(ozks) = directory.retrieve_current_ozks().await {
        match o_epoch {
            Some(epoch) => Some(directory.get_root_hash_at_epoch::<H>(&ozks, epoch).await),
            None => Some(directory.get_root_hash::<H>(&ozks).await),
        }
    } else {
        None
    }
    ...
    while let Some(Rpc(message, channel)) = rx.recv().await {
        match (message, channel) {
            ...
            (DirectoryCommand::Publish(a, b), Some(response)) => {
                ...
                match directory
                    .publish::<H>(vec![(
                        VkdLabel::from_utf8_str(&a),
                        VkdValue::from_utf8_str(&b),
                    )])
                    .await
                {
            ...
            (DirectoryCommand::Lookup(a), Some(response)) => {
                match directory.lookup::<H>(VkdLabel::from_utf8_str(&a)).await {
                    Ok(proof) => {
                        let hash = get_root_hash::<_, H, V>(directory, None).await;
                        match hash {
                            Some(Ok(root_hash)) => {
                                let vrf_pk = directory.get_public_key().await.unwrap();
                                let verification = vkd::client::lookup_verify::<H>(
                                    &vrf_pk,
                                    root_hash,
                                    VkdLabel::from_utf8_str(&a),
                                    proof,
            ...
            (DirectoryCommand::KeyHistory(a), Some(response)) => {
                match directory
                    .key_history::<H>(&VkdLabel::from_utf8_str(&a))
                    .await
                {
                    Ok(_proof) => {
