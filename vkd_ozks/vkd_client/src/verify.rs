...
fn hash_leaf_with_value(value: &crate::VkdValue, epoch: u64, proof: &[u8]) -> Digest {
...
pub fn key_history_verify(
    ...
    for (i, ver) in (last_version + 1..(1 << next_marker)).enumerate() {
        ...
        #[cfg(feature = "vrf")]
        {
            ...
            verify_vrf(vrf_public_key, &vkd_key, false, ver, vrf_pf, ver_label)?;
        }
        if !verify_nonmembership(root_hash, pf)? {
            return Err(VerificationError {error_message:
                    ...
                    &vkd_key, ver, current_epoch), error_type: VerificationErrorType::HistoryProof});
        }
    }
