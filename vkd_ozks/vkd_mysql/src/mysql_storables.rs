...
use vkd::storage::types::{DbRecord, StorageType};
use vkd::storage::Storable;
use vkd::tree_node::{NodeKey, TreeNode};
use vkd::NodeLabel;
...
pub(crate) const TABLE_AZKS: &str = "ozks";
...
const SELECT_HISTORY_TREE_NODE_DATA: &str =
    "`label_len`, `label_val`, `last_epoch`, `least_descendant_ep`, `parent_label_len`, `parent_label_val`, `node_type`, `left_child_len`, `left_child_label_val`, `right_child_len`, `right_child_label_val`, `hash`";
...
impl MySqlStorable for DbRecord {
    fn set_statement(&self) -> String {
        match &self {
            DbRecord::Ozks(_) => format!("INSERT INTO `{}` (`key`, {}) VALUES (:key, :epoch, :num_nodes) ON DUPLICATE KEY UPDATE `epoch` = :epoch, `num_nodes` = :num_nodes", TABLE_AZKS, SELECT_AZKS_DATA),
    ...
    fn set_params(&self) -> Option<mysql_async::Params> {
        match &self {
            DbRecord::Ozks(ozks) => Some(
                params! { "key" => 1u8, "epoch" => ozks.latest_epoch, "num_nodes" => ozks.num_nodes },
            ),
            DbRecord::TreeNode(node) => Some(params! {
                "label_len" => node.label.label_len,
                "label_val" => node.label.label_val,
                "last_epoch" => node.last_epoch,
                "least_descendant_ep" => node.least_descendant_ep,
    ...
    fn set_batch_statement<St: Storable>(items: usize) -> String {
        ...
        match St::data_type() {
            StorageType::Ozks => format!("INSERT INTO `{}` (`key`, {}) VALUES (:key, :epoch, :num_nodes) as new ON DUPLICATE KEY UPDATE `epoch` = new.epoch, `num_nodes` = new.num_nodes", TABLE_AZKS, SELECT_AZKS_DATA),
    ...
    fn set_batch_params(items: &[DbRecord]) -> Result<mysql_async::Params> {
        let param_batch = items
            ...
            .map(|(idx, item)| match &item {
                DbRecord::Ozks(ozks) => Ok(vec![
                    ("key".to_string(), Value::from(1u8)),
                    ("epoch".to_string(), Value::from(ozks.latest_epoch)),
                    ("num_nodes".to_string(), Value::from(ozks.num_nodes)),
                ]),
                DbRecord::TreeNode(node) => Ok(vec![
                    ...
                    (
                        ...
                        Value::from(node.least_descendant_ep),
                    ),
                    (
    ...
    fn get_statement<St: Storable>() -> String {
        match St::data_type() {
            StorageType::Ozks => format!("SELECT {} FROM `{}`", SELECT_AZKS_DATA, TABLE_AZKS),
    ...
    fn get_specific_params<St: Storable>(key: &St::StorageKey) -> Option<mysql_async::Params> {
        match St::data_type() {
            ...
            StorageType::ValueState => {
                let bin = St::get_full_binary_key_id(key);
                if let Ok(back) = vkd::storage::types::ValueState::key_from_full_binary(&bin) {
                    Some(params! {
                        "username" => back.0,
                        "epoch" => back.1
                    })
            ...
            StorageType::ValueState => {
                let pvec = keys
                    ...
                    .map(|(idx, key)| {
                        ...
                        // Since these are constructed from a safe key, they should never fail
                        // so we'll leave the unwrap to simplify
                        let back: vkd::storage::types::ValueStateKey =
                            vkd::storage::types::ValueState::key_from_full_binary(&bin).unwrap();
                        vec![
        ...
        match St::data_type() {
            StorageType::Ozks => {
                ...
                if let (Some(Ok(epoch)), Some(Ok(num_nodes))) = (row.take_opt(0), row.take_opt(1)) {
                    let ozks = DbRecord::build_ozks(epoch, num_nodes);
                    return Ok(DbRecord::Ozks(ozks));
                }
            }
            StorageType::TreeNode => {
