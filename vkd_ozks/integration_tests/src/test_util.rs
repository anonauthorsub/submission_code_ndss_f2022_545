...
>(
    ...
    match maybe_dir {
        ...
        Ok(dir) => {
            info!("VKD Directory started. Beginning tests");
            ...
            for i in 1..=3 {
                ...
                let ozks = dir.retrieve_current_ozks().await.unwrap();
                root_hashes.push(dir.get_root_hash::<Blake3>(&ozks).await);
            }

            // Perform 10 random lookup proofs on the published users
            let ozks = dir.retrieve_current_ozks().await.unwrap();
            let root_hash = dir.get_root_hash::<Blake3>(&ozks).await.unwrap();
                ...
                match dir.key_history::<Blake3>(&key).await {
                    ...
                    Ok(proof) => {
                        let (root_hash, current_epoch) =
                            vkd::directory::get_directory_root_hash_and_ep::<_, Blake3, V>(&dir)
                                .await
                                .unwrap();
                        let vrf_pk = dir.get_public_key().await.unwrap();
            ...
            match dir.audit::<Blake3>(1u64, 2u64).await {
                ...
                Ok(proof) => {
                    ...
                    match (start_root_hash, end_root_hash) {
                        (Ok(start), Ok(end)) => {
                            if let Err(error) =
                                vkd::auditor::audit_verify(vec![*start, *end], proof).await
                            {
...
pub(crate) async fn test_lookups<S: vkd::storage::Storage + Sync + Send, V: VRFKeyStorage>(
    mysql_db: &S,
    vrf: &V,
    num_users: u64,
    num_epochs: u64,
    ...
    match maybe_dir {
        ...
        Ok(dir) => {
            ...
            for user in users.iter().choose_multiple(&mut rng, num_lookups) {
                let label = VkdLabel::from_utf8_str(user);
                labels.push(label);
            }
            ...
            for label in labels.clone() {
                match dir.lookup::<Blake3>(label.clone()).await {
                    ...
                    Ok(proof) => {
                        let vrf_pk = dir.get_public_key().await.unwrap();
                        if let Err(error) =
                            vkd::client::lookup_verify::<Blake3>(&vrf_pk, root_hash, label, proof)
                        {
                            panic!("Lookup proof failed to verify {:?}", error);
                        }
                    }
...
// bulk lookup proofs.
async fn reset_mysql_db<S: vkd::storage::Storage + Sync + Send>(mysql_db: &S) {
