...
use vkd::storage::types::{VkdLabel, VkdValue, DbRecord};
use vkd::storage::StorageUtil;
use clap::Parser;
...
pub struct Delta {
    pub epoch: u32,
    pub updates: Vec<(VkdLabel, VkdValue)>,
}
...
pub(crate) async fn generate(args: Args) {
    ...
    let vrf = vkd::ecvrf::HardCodedVkdVRF {};
    let vkd = Directory::<_, _>::new::<Blake3>(&db, &vrf, false)
        .await
        .unwrap();
    ...
    for epoch in 1..=args.epochs {
        ...
        for (label, events) in user_map.iter() {
            if let Some(maybe_value) = events.get(&epoch) {
                let value = maybe_value
                    .clone()
                    .unwrap_or_else(|| VkdValue::random(&mut rng));
        ...
        if !args.no_generated_updates {
            ...
            for _ in updates.len()..num_updates as usize {
                updates.push((VkdLabel::random(&mut rng), VkdValue::random(&mut rng)));
            }
        }
        ...
        // perform publish
        vkd.publish::<Blake3>(updates.clone()).await.unwrap();
