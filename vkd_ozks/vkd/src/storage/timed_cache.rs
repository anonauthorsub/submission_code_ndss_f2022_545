...
pub struct TimedCache {
    ozks: Arc<tokio::sync::RwLock<Option<DbRecord>>>,
...
impl TimedCache {
    ...
    pub fn new(o_lifetime: Option<Duration>) -> Self {
        ...
        Self {
            ozks: Arc::new(tokio::sync::RwLock::new(None)),
    ...
    pub async fn hit_test<St: Storable>(&self, key: &St::StorageKey) -> Option<DbRecord> {
        ...
        if full_key
            == crate::ordered_append_only_zks::Ozks::get_full_binary_key_id(
                &crate::ordered_append_only_zks::DEFAULT_AZKS_KEY,
            )
        {
            // someone's requesting the AZKS object, return it from the special "cache" storage
            let record = self.ozks.read().await.clone();
    ...
    pub async fn put(&self, record: &DbRecord) {
        ...
        // special case for AZKS
        if let DbRecord::Ozks(ozks_ref) = &record {
            let mut guard = self.ozks.write().await;
            *guard = Some(DbRecord::Ozks(ozks_ref.clone()));
        } else {
    ...
    pub async fn batch_put(&self, records: &[DbRecord]) {
        ...
        for record in records.iter() {
            if let DbRecord::Ozks(ozks_ref) = &record {
                let mut ozks_guard = self.ozks.write().await;
                *ozks_guard = Some(DbRecord::Ozks(ozks_ref.clone()));
            } else {
                let key = record.get_full_binary_id();
                let item = CachedItem {
                    expiration: Instant::now() + self.item_lifetime,
