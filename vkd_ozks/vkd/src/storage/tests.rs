...
//! Test utilities of storage layers implementing the storage primatives for VKD
...
type Ozks = crate::ordered_append_only_zks::Ozks;
...
async fn test_get_and_set_item<Ns: Storage>(storage: &Ns) {
    ...
    let set_result = storage.set(DbRecord::Ozks(ozks.clone())).await;
    assert_eq!(Ok(()), set_result);

    let get_result = storage
        .get::<Ozks>(&crate::ordered_append_only_zks::DEFAULT_AZKS_KEY)
        .await;
    if let Ok(DbRecord::Ozks(got_ozks)) = get_result {
        assert_eq!(got_ozks.latest_epoch, ozks.latest_epoch);
        assert_eq!(got_ozks.num_nodes, ozks.num_nodes);
    } else {
    ...
    let value = ValueState {
        username: VkdLabel::from_utf8_str("test"),
        epoch: 1,
        label: NodeLabel::new(byte_arr_from_u64(1), 1),
        version: 1,
        plaintext_val: VkdValue::from_utf8_str("abc123"),
    };
    ...
    for value in rand_users.iter() {
        for user in rand_users.iter() {
            data.push(DbRecord::ValueState(ValueState {
                plaintext_val: VkdValue(value.clone()),
                version: epoch,
                label: NodeLabel {
...
async fn test_user_data<S: Storage + Sync + Send>(storage: &S) {
    ...
    let mut sample_state = ValueState {
        plaintext_val: VkdValue(rand_value.clone()),
        version: 1u64,
        label: NodeLabel {
            label_val: byte_arr_from_u64(1),
            label_len: 1u32,
        ...
        epoch: 1u64,
        username: VkdLabel(rand_user),
    };
    let mut sample_state_2 = sample_state.clone();
    sample_state_2.username = VkdLabel::from_utf8_str("test_user");
...
async fn test_tombstoning_data<S: Storage + Sync + Send>(
    ...
    let mut sample_state = ValueState {
        ...
        epoch: 1u64,
        username: VkdLabel(rand_user.clone()),
    };
    let mut sample_state2 = sample_state.clone();
    sample_state2.username = VkdLabel::from_utf8_str("tombstone_test_user");
    ...
    for label in [
        VkdLabel::from_utf8_str("tombstone_test_user"),
        VkdLabel(rand_user),
    ] {
        for version in 0..5 {
