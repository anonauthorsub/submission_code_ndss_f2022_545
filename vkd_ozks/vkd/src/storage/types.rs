...
impl VkdLabel {
    /// Build an [`VkdLabel`] struct from an UTF8 string
    pub fn from_utf8_str(value: &str) -> Self {
        Self(value.as_bytes().to_vec())
    }
}

impl core::ops::Deref for VkdLabel {
    type Target = [u8];
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
/// The types of value used in the key-value pairs of a VKD
...
impl VkdValue {
    /// Build an [`VkdValue`] struct from an UTF8 string
    pub fn from_utf8_str(value: &str) -> Self {
        Self(value.as_bytes().to_vec())
    }
}

impl core::ops::Deref for VkdValue {
    type Target = [u8];
    fn deref(&self) -> &Self::Target {
        &self.0
    }
...
pub struct ValueState {
    ...
    pub plaintext_val: VkdValue, // This needs to be the plaintext value, to discuss
...
#[cfg_attr(feature = "serde_serialization", serde(bound = ""))]
pub struct KeyData {
    /// The vector of states of key data for a given VkdLabel
...
impl Clone for DbRecord {
    fn clone(&self) -> Self {
        match &self {
            DbRecord::Ozks(ozks) => DbRecord::Ozks(ozks.clone()),
...
impl DbRecord {
    ...
    pub fn get_full_binary_id(&self) -> Vec<u8> {
        match &self {
            DbRecord::Ozks(ozks) => ozks.get_full_binary_id(),
    ...
    /// An Ozks record should always be updated last, so that any concurrent storage readers will
    ...
    /// Build an ozks instance from the properties
    pub fn build_ozks(latest_epoch: u64, num_nodes: u64) -> Ozks {
        Ozks {
            latest_epoch,
            num_nodes,
        }
    ...
    pub fn build_user_state(
        ...
        ValueState {
            plaintext_val: VkdValue(plaintext_val),
            version,
            label: NodeLabel::new(label_val, label_len),
            epoch,
            username: VkdLabel(username),
        }
    }
}
