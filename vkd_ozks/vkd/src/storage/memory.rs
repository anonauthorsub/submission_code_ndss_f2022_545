...
//! This module contains an in-memory database for the VKD library as well as
...
use crate::storage::types::{
    VkdLabel, VkdValue, DbRecord, KeyData, StorageType, ValueState, ValueStateKey,
...
impl Storage for AsyncInMemoryDatabase {
        ...
        for record in data {
            if let DbRecord::ValueState(value_state) = record {
                ...
                new_data.push(DbRecord::ValueState(ValueState {
                    plaintext_val: crate::VkdValue(crate::TOMBSTONE.to_vec()),
                    ..value_state
                }));
            }
        }
    ...
    async fn get_user_state_versions(
        ...
        keys: &[VkdLabel],
        flag: ValueStateRetrievalFlag,
    ) -> Result<HashMap<VkdLabel, (u64, VkdValue)>, StorageError> {
        let mut map = HashMap::new();
        for username in keys.iter() {
            if let Ok(result) = self.get_user_state(username, flag).await {
                map.insert(
                    VkdLabel(result.username.to_vec()),
                    (result.version, VkdValue(result.plaintext_val.to_vec())),
                );
            }
        }
        Ok(map)
    ...
    async fn batch_get_type_direct<St: Storable>(&self) -> Result<Vec<DbRecord>, StorageError> {
        let records = self
            ...
            .filter(|record| match record {
                DbRecord::Ozks(_) => St::data_type() == StorageType::Ozks,
