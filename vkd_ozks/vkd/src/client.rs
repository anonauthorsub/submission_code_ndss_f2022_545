...
pub fn verify_membership<H: Hasher>(
    ...
    if proof.label.label_len == 0 {
        ...
        if final_hash == root_hash {
            ...
            return Err(VkdError::OzksErr(OzksError::VerifyMembershipProof(
...
pub fn verify_nonmembership<H: Hasher>(
    ...
    if !verified {
        return Err(VkdError::Directory(DirectoryError::VerifyLookupProof(
...
pub fn key_history_verify<H: Hasher>(
    ...
    if num_proofs == 0 {
        return Err(VkdError::Directory(DirectoryError::VerifyKeyHistoryProof(
            format!(
                "No update proofs included in the proof of user {:?} at epoch {:?}!",
                vkd_key, current_epoch
            ),
        )));
    }
...
fn verify_single_update_proof<H: Hasher>(
    ...
    if version > 1 {
        ...
        let previous_null_err = VkdError::Directory(DirectoryError::VerifyKeyHistoryProof(err_str));
        ...
        let vrf_previous_null_err =
            VkdError::Directory(DirectoryError::VerifyKeyHistoryProof(vrf_err_str));
...
fn hash_leaf_with_value<H: Hasher>(value: &crate::VkdValue, epoch: u64, proof: &[u8]) -> H::Digest {
    let single_hash = crate::utils::bind_commitment::<H>(value, proof);
    H::merge_with_int(single_hash, epoch)
}

#[allow(unused)]
fn hash_plaintext_value<H: Hasher>(value: &crate::VkdValue, proof: &[u8]) -> H::Digest {
