...
impl From<TreeNodeError> for VkdError {
...
impl From<VrfError> for VkdError {
    fn from(error: VrfError) -> Self {
        Self::Vrf(error)
    }
}

impl From<OzksError> for VkdError {
    fn from(error: OzksError) -> Self {
        Self::OzksErr(error)
    }
}

impl From<AuditorError> for VkdError {
...
impl std::fmt::Display for VkdError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        match self {
            VkdError::TreeNode(err) => {
                writeln!(f, "VKD Tree Node Error: {}", err)
            }
            VkdError::Directory(err) => {
                writeln!(f, "VKD Directory Error: {}", err)
            }
            VkdError::OzksErr(err) => {
                writeln!(f, "VKD AZKS Error: {}", err)
            }
            VkdError::Vrf(err) => {
                writeln!(f, "VKD VRF Error: {}", err)
            }
            VkdError::Storage(err) => {
                writeln!(f, "VKD Storage Error: {}", err)
            }
            VkdError::AuditErr(err) => {
                writeln!(f, "VKD Auditor Error {}", err)
            }
            VkdError::TestErr(err) => {
                writeln!(f, "{}", err)
            }
        }
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidDirection(dir) => {
                write!(
                    f,
                    "VKD is based on a binary tree. No child with a given index: {}",
                    dir
                )
            }
...
/// An error thrown by the Ozks data structure.
#[cfg_attr(test, derive(PartialEq))]
#[derive(Debug)]
pub enum OzksError {
...
impl std::error::Error for OzksError {}
