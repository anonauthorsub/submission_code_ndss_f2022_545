...
impl Storable for Ozks {
    ...
    fn get_full_binary_key_id(key: &u8) -> Vec<u8> {
        vec![StorageType::Ozks as u8, *key]
    }

    fn key_from_full_binary(bin: &[u8]) -> Result<u8, String> {
        if bin.is_empty() || bin[0] != StorageType::Ozks as u8 {
...
impl Ozks {
    /// Creates a new ozks
    pub async fn new<S: Storage + Sync + Send, H: Hasher>(storage: &S) -> Result<Self, VkdError> {
    ...
    /// An ozks is built both by the [crate::directory::Directory] and the auditor.
    ...
    /// the ozks tree that remain unchanged from `start_epoch` to `end_epoch` and the leaves inserted into the
    /// tree after `start_epoch` and  up until `end_epoch`.
    /// If there is no errors, this function returns an `Ok` result, containing the
    ///  append-only proof and otherwise, it returns a [errors::VkdError].
    ///
    ...
    async fn get_append_only_proof_helper<S: Storage + Sync + Send, H: Hasher>(
        ...
        start_epoch: u64,
        end_epoch: u64,
    ) -> Result<AppendOnlyHelper<H>, VkdError> {
    ...
    /// Gets the root hash for this ozks
    ...
    /// the ozks does, this would never be called at an epoch before the birth of the root node.
    pub async fn get_root_hash_at_epoch<S: Storage + Sync + Send, H: Hasher>(
        ...
        if self.latest_epoch < epoch {
            ...
            return Err(VkdError::TreeNode(TreeNodeError::NonexistentAtEpoch(
                NodeLabel::root(),
                epoch,
            )));
        }
    ...
    /// Gets the latest epoch of this ozks. If an update aka epoch transition
    ...
    pub async fn get_membership_proof_and_node<S: Storage + Sync + Send, H: Hasher>(
        ...
        storage: &S,
        label: NodeLabel,
    ) -> Result<(MembershipProof<H>, NodeLabel), VkdError> {
        ...
        while !equal && dir.is_some() {
            ...
            let mut count = 0;
            let direction = dir.ok_or(VkdError::TreeNode(TreeNodeError::NoDirection(
                curr_node.label,
                None,
            )))?;
            ...
            if next_state.is_some() {
                for i in 0..ARITY {
                    let no_direction_error =
                        VkdError::TreeNode(TreeNodeError::NoDirection(curr_node.label, None));
...
mod tests {
    ...
    async fn test_batch_insert_basic() -> Result<(), VkdError> {
        ...
        let mut ozks1 = Ozks::new::<_, Blake3>(&db).await?;
        ...
        for _ in 0..num_nodes {
            ...
            ozks1.insert_leaf::<_, Blake3>(&db, node, 1).await?;
        }

        let db2 = AsyncInMemoryDatabase::new();
        let mut ozks2 = Ozks::new::<_, Blake3>(&db2).await?;
        ...
        assert_eq!(
            ozks1.get_root_hash::<_, Blake3>(&db).await?,
            ozks2.get_root_hash::<_, Blake3>(&db2).await?,
    ...
    async fn test_membership_proof_permuted() -> Result<(), VkdError> {
        ...
        insertion_set.shuffle(&mut rng);
        let db = AsyncInMemoryDatabase::new();
        let mut ozks = Ozks::new::<_, Blake3>(&db).await?;
        ozks.batch_insert_leaves::<_, Blake3>(&db, insertion_set.clone())
            .await?;
        ...
        verify_membership::<Blake3>(ozks.get_root_hash::<_, Blake3>(&db).await?, &proof)?;

        Ok(())
    }

    #[tokio::test]
    async fn test_membership_proof_small() -> Result<(), VkdError> {
        let num_nodes = 2;
    ...
    async fn test_membership_proof_intermediate() -> Result<(), VkdError> {
        ...
        let mut ozks = Ozks::new::<_, Blake3>(&db).await?;
        ozks.batch_insert_leaves::<_, Blake3>(&db, insertion_set)
            .await?;
        let search_label = NodeLabel::new(byte_arr_from_u64(0b1111 << 60), 64);
        let proof = ozks.get_non_membership_proof(&db, search_label).await?;
        assert!(
            verify_nonmembership::<Blake3>(ozks.get_root_hash::<_, Blake3>(&db).await?, &proof)?,
    ...
    #[tokio::test]
    async fn test_nonmembership_proof_very_small() -> Result<(), VkdError> {
        let num_nodes = 2;
        ...
        ozks.batch_insert_leaves::<_, Blake3>(&db, insertion_set.clone()[1..2].to_vec())
            .await?;
        let proof = ozks.get_non_membership_proof(&db, search_label).await?;
    ...
    // verifies.
    #[tokio::test]
    async fn test_nonmembership_proof_small() -> Result<(), VkdError> {
        let num_nodes = 3;
        let mut rng = OsRng;
        ...
        ozks.batch_insert_leaves::<_, Blake3>(
            &db,
    ...
    async fn test_append_only_proof_very_tiny() -> Result<(), VkdError> {
        ...
        ozks.batch_insert_leaves::<_, Blake3>(&db, insertion_set_1)
            .await?;
        let start_hash = ozks.get_root_hash::<_, Blake3>(&db).await?;
        ...
        ozks.batch_insert_leaves::<_, Blake3>(&db, insertion_set_2)
            .await?;
        let end_hash = ozks.get_root_hash::<_, Blake3>(&db).await?;

        let proof = ozks.get_append_only_proof(&db, 1, 2).await?;
    ...
    async fn test_append_only_proof() -> Result<(), VkdError> {
        ...
        let db = AsyncInMemoryDatabase::new();
        let mut ozks = Ozks::new::<_, Blake3>(&db).await?;
        ozks.batch_insert_leaves::<_, Blake3>(&db, insertion_set_1.clone())
            .await?;
        ...
        ozks.batch_insert_leaves::<_, Blake3>(&db, insertion_set_2.clone())
            .await?;

        let middle_hash = ozks.get_root_hash::<_, Blake3>(&db).await?;
        ...
        ozks.batch_insert_leaves::<_, Blake3>(&db, insertion_set_3.clone())
            .await?;

        let end_hash = ozks.get_root_hash::<_, Blake3>(&db).await?;

        let proof = ozks.get_append_only_proof(&db, 1, 3).await?;
    ...
    #[tokio::test]
    async fn future_epoch_throws_error() -> Result<(), VkdError> {
        let db = AsyncInMemoryDatabase::new();
        let ozks = Ozks::new::<_, Blake3>(&db).await?;

        let out = ozks.get_root_hash_at_epoch::<_, Blake3>(&db, 123).await;

        let expected = Err::<_, VkdError>(VkdError::TreeNode(TreeNodeError::NonexistentAtEpoch(
            NodeLabel::root(),
            123,
        )));
