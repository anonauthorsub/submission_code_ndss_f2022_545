...
use crate::errors::{VkdError, StorageError, TreeNodeError};
...
impl TreeNode {
    ...
    >(
        ...
        match child_node {
            ...
            None => Err(VkdError::TreeNode(TreeNodeError::NoChildAtEpoch(
                epoch,
                dir_leaf.unwrap(),
            ))),
        }
    ...
    pub(crate) async fn get_child_state<S: Storage + Sync + Send>(
        ...
        storage: &S,
        direction: Direction,
    ) -> Result<Option<TreeNode>, VkdError> {
        match direction {
            Direction::None => Err(VkdError::TreeNode(TreeNodeError::NoDirection(
                self.label, None,
            ))),
            Direction::Some(_dir) => {
    ...
    pub(crate) fn get_child(&self, direction: Direction) -> Result<Option<NodeLabel>, VkdError> {
        match direction {
            Direction::None => Err(VkdError::TreeNode(TreeNodeError::NoDirection(
                self.label, None,
            ))),
            Direction::Some(dir) => {
                ...
                if dir == 0 {
                    ...
                    Err(VkdError::TreeNode(TreeNodeError::InvalidDirection(dir)))
                }
            }
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_least_descendant_ep() -> Result<(), VkdError> {
