...
use vkd::history_tree_node::HistoryTreeNode;
use vkd::node_state::HistoryNodeState;
use vkd::storage::types::{
    VkdLabel, VkdValue, DbRecord, KeyData, StorageType, ValueState, ValueStateRetrievalFlag,
};
...
type LocalTransaction = vkd::storage::transaction::Transaction;

use vkd::storage::timed_cache::*;
    ...
    async fn internal_batch_set(
        ...
        *(self.num_writes.write().await) += records.len() as u64;
        ...
        let statement = |i: usize| -> String {
            match &head {
                DbRecord::Azks(_) => DbRecord::set_batch_statement::<vkd::Azks>(i),
                ...
                DbRecord::ValueState(_) => {
                    DbRecord::set_batch_statement::<vkd::storage::types::ValueState>(i)
                }
            }
        };
    ...
    fn try_dockers() -> std::io::Result<std::process::Output> {
        ...
        for path in potential_docker_paths {
            output = Command::new(path)
                ...
                // Therefore, a container with a name like vkd-test-dbc would match but would be wrong.
                // This regex ensures exact match.
                .args(["container", "ls", "-f", "name=^/vkd-test-db$"])
                .output();
            match &output {
                Ok(result) => {
...
impl Storage for AsyncMySqlDatabase {
    ...
    async fn tombstone_value_states(
        &self,
        keys: &[vkd::storage::types::ValueStateKey],
        ...
        for record in data {
            if let DbRecord::ValueState(value_state) = record {
                new_data.push(DbRecord::ValueState(ValueState {
                    ...
                    plaintext_val: vkd::VkdValue(vkd::TOMBSTONE.to_vec()),
                    ...
                    if let (
                        ...
                        if node_label_val.len() == 32 {
                            ...
                            return Some(ValueState {
                                ...
                                plaintext_val: vkd::storage::types::VkdValue(data),
                                username: vkd::storage::types::VkdLabel(username),
                            });
                        }
                    }
                    None
            ...
            if self.is_transaction_active().await {
                ...
                for record in selected_records.into_iter() {
                    if let Some(DbRecord::ValueState(value)) = self
                        .trans
                        .get::<vkd::storage::types::ValueState>(&record.get_id())
                        .await
                    {
                        updated.push(value);
                    } else {
            ...
            *(self.time_read.write().await) += toc;
            ...
            if self.is_transaction_active().await {
                if let Some(found_item) = &item {
                    if let Some(DbRecord::ValueState(value)) = self
                        .trans
                        .get::<vkd::storage::types::ValueState>(&found_item.get_id())
                        .await
                    {
    ...
    async fn get_user_state_versions(
        ...
        keys: &[VkdLabel],
        flag: ValueStateRetrievalFlag,
    ) -> core::result::Result<HashMap<VkdLabel, (u64, VkdValue)>, StorageError> {
        *(self.num_reads.write().await) += 1;
            ...
            let out = if params_map.is_empty() {
                ...
                self.check_for_infra_error(_t)?
                    .reduce_and_drop(vec![], |mut acc, mut row: mysql_async::Row| {
                        ...
                        {
                            acc.push((VkdLabel(username), (version, VkdValue(data))))
                        }
                        acc
                    })
                    .await?
