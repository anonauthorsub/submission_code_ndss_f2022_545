...
fn convert_label(proof: vkd::node_state::NodeLabel) -> crate::types::NodeLabel {
...
async fn test_simple_lookup() -> Result<(), VkdError> {
    ...
    let root_hash = vkd.get_root_hash::<Hash>(&current_azks).await?;
...
//         internal_lookup_proof,
//     )
//     .map_err(|i_err| VkdError::AzksNotFound(format!("Internal: {:?}", i_err)));
...
async fn test_history_proof_multiple_epochs() -> Result<(), VkdError> {
    ...
    let (mut root_hashes, previous_root_hashes) =
        vkd::directory::get_key_history_hashes::<_, Hash, HardCodedVkdVRF>(&vkd, &proof)
            .await
            .unwrap();
...
async fn test_tombstoned_key_history() -> Result<(), VkdError> {
    ...
    let (root_hashes, previous_root_hashes) =
        vkd::directory::get_key_history_hashes(&vkd, &history_proof).await?;
