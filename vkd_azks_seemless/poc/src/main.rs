...
use vkd::Directory;
use vkd_mysql::mysql::{AsyncMySqlDatabase, MySqlCacheOptions};
...
async fn main() {
    ...
    let level = if cli.debug {
        ...
        match logs::FileLogger::new("vkd_app.log") {
    ...
    if cli.memory_db {
        let db = vkd::storage::memory::AsyncInMemoryDatabase::new();
        ...
        tokio::spawn(async move {
            directory_host::init_host::<_, Blake3, HardCodedVkdVRF>(&mut rx, &mut directory).await
        });
        ...
        match other_mode {
            OtherMode::BenchDbInsert { num_users } => {
                ...
                for value in values.iter() {
                    let state = vkd::storage::types::DbRecord::build_user_state(
                    ...
                    data.push(vkd::storage::types::DbRecord::ValueState(state));
                }
        ...
        loop {
            ...
            match Command::parse(&mut line) {
                ...
                Command::Info => {
                    ...
                    println!("===== Verifiable Key Directory Information =====");
