...
use crate::errors::{VkdError, HistoryTreeNodeError};
...
pub fn to_digest<H: Hasher>(input: &[u8]) -> Result<H::Digest, VkdError> {
...
mod tests {
    ...
    #[tokio::test]
    pub async fn lookup_proof_roundtrip() -> Result<(), VkdError> {
        let db = AsyncInMemoryDatabase::new();

        let vrf = HardCodedVkdVRF {};
        let vkd = Directory::<_, _>::new::<Blake3_256<BaseElement>>(&db, &vrf, false)
            .await
            .unwrap();
        vkd.publish::<Blake3_256<BaseElement>>(vec![
            (
        ...
        let lookup_proof = vkd
            .lookup::<Blake3_256<BaseElement>>(VkdLabel::from_utf8_str("hello"))
            .await
            .unwrap();
    ...
    #[tokio::test]
    pub async fn history_proof_roundtrip() -> Result<(), VkdError> {
        let db = AsyncInMemoryDatabase::new();
        let vrf = HardCodedVkdVRF {};
        let vkd = Directory::<_, _>::new::<Blake3_256<BaseElement>>(&db, &vrf, false)
            .await
        ...
        let history_proof = vkd
            .key_history::<Blake3_256<BaseElement>>(&VkdLabel::from_utf8_str("hello"))
            .await
            .unwrap();
    ...
    #[tokio::test]
    pub async fn audit_proof_roundtrip() -> Result<(), VkdError> {
        let db = AsyncInMemoryDatabase::new();
        let vrf = HardCodedVkdVRF {};
        let vkd = Directory::<_, _>::new::<Blake3_256<BaseElement>>(&db, &vrf, false)
            .await
