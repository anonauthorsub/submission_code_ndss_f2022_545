...
use crate::errors::{VkdError, HistoryTreeNodeError, StorageError};
...
impl HistoryTreeNode {
    ...
    async fn update_hash_at_parent<S: Storage + Sync + Send, H: Hasher>(
        ...
        match get_state_map(storage, parent, epoch).await {
            Err(_) => Err(VkdError::HistoryTreeNode(
            ...
            Ok(parent_state) => match parent.get_direction_at_ep(storage, self, epoch).await? {
                None => Err(VkdError::HistoryTreeNode(
    ...
    pub(crate) async fn get_child_at_epoch<S: Storage + Sync + Send, H: Hasher>(
        ...
        epoch: u64,
        direction: Direction,
    ) -> Result<Option<HistoryChildState>, VkdError> {
        match direction {
            Direction::None => Err(VkdError::HistoryTreeNode(
    ...
    pub(crate) async fn get_state_at_epoch<S: Storage + Sync + Send>(
        ...
        storage: &S,
        epoch: u64,
    ) -> Result<HistoryNodeState, VkdError> {
        if self.get_birth_epoch() > epoch {
            Err(VkdError::HistoryTreeNode(
    ...
    async fn to_node_unhashed_child_state<S: Storage + Sync + Send, H: Hasher>(
        &self,
        storage: &S,
    ) -> Result<HistoryChildState, VkdError> {
...
pub async fn get_empty_root<H: Hasher, S: Storage + Send + Sync>(
    storage: &S,
    ep: Option<u64>,
) -> Result<HistoryTreeNode, VkdError> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_set_child_without_hash_at_root() -> Result<(), VkdError> {
        let ep = 1;
    ...
    #[tokio::test]
    async fn test_set_children_without_hash_at_root() -> Result<(), VkdError> {
        let ep = 1;
        let db = InMemoryDb::new();
        let mut root = get_empty_root::<Blake3, _>(&db, Option::Some(ep)).await?;
        let child_hist_node_1 = HistoryChildState::new::<Blake3>(
    ...
    #[tokio::test]
    async fn test_set_children_without_hash_multiple_at_root() -> Result<(), VkdError> {
        let mut ep = 1;
        let db = InMemoryDb::new();
        let mut root = get_empty_root::<Blake3, _>(&db, Option::Some(ep)).await?;
        let child_hist_node_1 = HistoryChildState::new::<Blake3>(
    ...
    #[tokio::test]
    async fn test_get_child_at_existing_epoch_multiple_at_root() -> Result<(), VkdError> {
        let mut ep = 1;
        let db = InMemoryDb::new();
        let mut root = get_empty_root::<Blake3, _>(&db, Option::Some(ep)).await?;
        let child_hist_node_1 = HistoryChildState::new::<Blake3>(
    ...
    #[tokio::test]
    pub async fn test_get_child_at_epoch_at_root() -> Result<(), VkdError> {
        let init_ep = 0;
        let db = InMemoryDb::new();
    ...
    #[tokio::test]
    async fn test_insert_single_leaf_root() -> Result<(), VkdError> {
        let db = InMemoryDb::new();
    ...
    #[tokio::test]
    async fn test_insert_single_leaf_below_root() -> Result<(), VkdError> {
        let db = InMemoryDb::new();
        let mut root = get_empty_root::<Blake3, _>(&db, Option::Some(0u64)).await?;
        let new_leaf = get_leaf_node::<Blake3, _>(
            &db,
    ...
    #[tokio::test]
    async fn test_insert_single_leaf_below_root_both_sides() -> Result<(), VkdError> {
        let db = InMemoryDb::new();
        let mut root = get_empty_root::<Blake3, _>(&db, Option::Some(0u64)).await?;
        let new_leaf = get_leaf_node::<Blake3, _>(
            &db,
    ...
    #[tokio::test]
    async fn test_insert_single_leaf_full_tree() -> Result<(), VkdError> {
        let db = InMemoryDb::new();
        let mut root = get_empty_root::<Blake3, _>(&db, Option::Some(0u64)).await?;
        root.write_to_storage(&db).await?;
        let mut num_nodes = 1;
