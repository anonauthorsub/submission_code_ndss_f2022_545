...
impl Azks {
    ...
    pub async fn get_root_hash_at_epoch<S: Storage + Sync + Send, H: Hasher>(
        ...
        if self.latest_epoch < epoch {
            ...
            return Err(VkdError::HistoryTreeNode(
    ...
    pub async fn get_membership_proof_and_node<S: Storage + Sync + Send, H: Hasher>(
        ...
        while !equal && dir.is_some() {
            ...
            let direction = dir.ok_or({
                VkdError::HistoryTreeNode(HistoryTreeNodeError::NoDirection(curr_node.label, None))
            })?;
            ...
            for i in 0..ARITY {
                let no_direction_error = VkdError::HistoryTreeNode(
...
mod tests {
    ...
    async fn future_epoch_throws_error() -> Result<(), VkdError> {
        ...
        let expected = Err::<_, VkdError>(VkdError::HistoryTreeNode(
