...
use crate::{
    ...
    errors::{VkdError, AzksError, DirectoryError},
...
pub fn verify_membership<H: Hasher>(
    ...
    if proof.label.len == 0 {
        ...
        if final_hash == root_hash {
            ...
            return Err(VkdError::AzksErr(AzksError::VerifyMembershipProof(
...
fn verify_single_update_proof<H: Hasher>(
    ...
    if epoch > 1 {
        ...
        verify_nonmembership(
            root_hash,
            non_existence_before_ep.as_ref().ok_or_else(|| VkdError::Directory(DirectoryError::VerifyKeyHistoryProof(format!(
...
fn build_and_hash_layer<H: Hasher>(
    ...
    let direction = dir.ok_or({
        VkdError::HistoryTreeNode(HistoryTreeNodeError::NoDirection(parent_label, None))
    })?;
