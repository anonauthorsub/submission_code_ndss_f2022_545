...
//! use akd::storage::types::{VkdLabel, VkdValue, DbRecord, ValueState, ValueStateRetrievalFlag};
//! use akd::storage::Storage;
//! use akd::storage::memory::AsyncInMemoryDatabase;
//! use akd::ecvrf::HardCodedVkdVRF;
...
//!     let mut akd = Directory::<_, HardCodedVkdVRF>::new::<Blake3_256<BaseElement>>(&db, &vrf, false).await.unwrap();
//! };
//! ```
//!
...
//! we use the `publish` function of an akd. The argument of publish is a vector of tuples of type (VkdLabel::from_utf8_str(String), VkdValue::from_utf8_str(String)). See below for example usage.
//! ```
//! use winter_crypto::Hasher;
//! use winter_crypto::hashers::Blake3_256;
//! use winter_math::fields::f128::BaseElement;
...
//!     akd.publish::<Blake3_256<BaseElement>>(vec![(VkdLabel::from_utf8_str("hello"), VkdValue::from_utf8_str("world")),
...
//!     let lookup_proof = akd.lookup::<Blake3_256<BaseElement>>(VkdLabel::from_utf8_str("hello")).await;
//! };
//! ```
...
//!          .await.unwrap();
//!     // Generate latest proof
//!     let lookup_proof = akd.lookup::<Blake3_256<BaseElement>>(VkdLabel::from_utf8_str("hello")).await.unwrap();
...
//!          .await.unwrap();
//!     // Generate latest proof
//!     let history_proof = akd.key_history::<Blake3_256<BaseElement>>(&VkdLabel::from_utf8_str("hello")).await;
//! };
//! ```
...
//!          .await.unwrap();
//!     // Generate latest proof
//!     let history_proof = akd.key_history::<Blake3_256<BaseElement>>(&VkdLabel::from_utf8_str("hello")).await.unwrap();
//!     let current_azks = akd.retrieve_current_azks().await.unwrap();
//!     // Get the azks root hashes at the required epochs
//!     let (root_hashes, previous_root_hashes) = akd::directory::get_key_history_hashes::<_, Blake3_256<BaseElement>, HardCodedVkdVRF>(&akd, &history_proof).await.unwrap();
//!     let vrf_pk = akd.get_public_key().await.unwrap();
...
//!     akd.publish::<Blake3_256<BaseElement>>(vec![(VkdLabel::from_utf8_str("hello3"), VkdValue::from_utf8_str("world3")),
